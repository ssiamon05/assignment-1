...
log_attendees_distribution <- boxplot(log10(num_attendees))
...
count_in_location <- function(location) {
  n <- length(str_match(locations, location))
  str_c("There were ",n," protests in ",location,".")
} 
...
# `stringr` package
states <- toupper(str_sub(locations, str_length(locations)-1,
                          str_length(locations)))
...
in_2020 <- dates[dates >= as.Date("2020-1-1")]
# Create a vector of the dates that are in 2019. `in_2019`
in_2019 <- dates[dates < as.Date("2020-1-1") & dates >= as.Date("2019-1-1")]
...
#This number honestly does not surprise me. With the amount of inequalities that
#have been put in the spotlight since 2020 started, it makes sense that
#double the amount of protests have taken place.
...
count_on_date <- function(date) {
  n <- length(dates[as.Date(date, format = "%B %dth, %Y") == dates])
  str_c("There were ",n," protests on ",date,".")
}
...
# July 2019? You'll want to do this in multiple stes as you see fit, though
...
num_purposes <- length(unique(tolower(purpose)))
View(purpose)
...
level_purpose <- 
  unique(str_replace(as.character(purpose),"\\s*\\([^\\)]+\\)",""))
mid_level_purpose <- unique(str_replace_all(as.character(level_purpose),
                          "[[:punct:]]", ""))
high_level_purpose <- unique(str_trim(mid_level_purpose, side = "right"))
...
#I will be making a function that shows the average amount of people at a
#protest in each state.

avg_in_state <- function(location) {
  n <- length(str_match(locations, tolower(location)))
  p <- sum(protests$Attendees[tolower(location)], na.rm = True)
  p/n
}
